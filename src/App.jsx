...
function isPEAEligible(asset) {
  if (!asset) return false;
  const symbol = (asset.symbol || "").toUpperCase();
  const exchange = (asset.exchange || "").toUpperCase();
  return (
    asset.quoteType === "EQUITY" &&
    (symbol.endsWith(".PA") || exchange.includes("PAR") || exchange.includes("EPA"))
  );
}
  ...
  return (
    <div className="bsl-app">
      <style>{`
        ...
        .bsl-type-pill.pea { background: rgba(52,217,160,0.12); color: var(--buy); border-color: rgba(52,217,160,0.3);}
      ...
      {combined && (
        <div className="bsl-stats">
          <StatCard label="Prix actuel" value={`${fmtPrice(currentPrice)} ${getCurrencySymbol(currency)}`} />
          <StatCard
            ...
            <span className="sub">
              {selected ? (
                <>
                  ...
                  {isPEAEligible(selected) && <span className="bsl-type-pill pea">PEA</span>}
                  <button
                    type="button"
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={series} margin={{ top: 8, right: 4, left: 0, bottom: 0 }}>
                  ...
                  <Tooltip
                    ...
                    formatter={(v) => [`${fmtPrice(v)} ${getCurrencySymbol(currency)}`, "Prix"]}
                  />
